    ...
    if(missing_info.length ===0){
        ...
        if (documentSection === 'Plans used to reach decision') {
            getPlansUsedToReachDecisionText()
                .should('be.visible')
                .type(missing_info);
            getPlansUsedToReachDecisionText().should('have.length.at.most',1000);
        }
        else if (documentSection === 'Statutory development plan policies'){
            getStatutoryDevelopmentPlanText()
                .should('be.visible')
                .type(missing_info);
            getStatutoryDevelopmentPlanText().should('have.length.at.most',1000);
        }
        else if (documentSection === 'Other relevant policies') {
            getOtherRelevantPoliciesText()
                .should('be.visible')
                .type(missing_info);
            getOtherRelevantPoliciesText().should('have.length.at.most',1000);
        }
        else if (documentSection === 'Supplementary planning documents'){
            getSupplementaryPlanningText()
                .should('be.visible')
                .type(missing_info);
            getSupplementaryPlanningText().should('have.length.at.most',1000);
        }
        else if (documentSection === 'Conservation area map and guidance'){
            getConservationAreaMapText()
                .should('be.visible')
                .type(missing_info);
            getConservationAreaMapText().should('have.length.at.most',1000);
        }
            ...
            getListingDescriptionText()
                .should('be.visible')
                .type(missing_info);
            getListingDescriptionText().should('have.length.at.most',1000);
        }
        else if (documentSection === 'Application notification') {
            ...
            getRepresentationsText()
                .should('be.visible')
                .type(missing_info);
            getRepresentationsText().should('have.length.at.most',1000);
        }
        else if (documentSection === 'Appeal notification') {
