...
if [ "${TF_BUILD:-}" != "True" ] && [ "${ALLOW_LOCAL_ENV_OVERWRITE:-}" != "true" ]; then
	echo "This script writes local .env and dummy data files. Set ALLOW_LOCAL_ENV_OVERWRITE=true to run it outside Azure Pipelines."
	exit 1
fi
...
PINS_FEATURE_FLAG_AZURE_CONNECTION_STRING=local-e2e
