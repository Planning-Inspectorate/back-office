Feature: Verify Search Functionality as a Case Team user
	...
	@CaseTeam @smoke
	Scenario: Case Team user should be able to use search using Case Reference- Open Cases
	...
	@CaseTeam @smoke
	Scenario: Case Team user should be able to use search using Case Name - Open Cases
		Given the user searches for a case using its "Case Name"
		Then the user should get "1" matching cases in the results
		And the top search result should be "BC0110001 - Office Use Test Application 1"

	@CaseTeam @smoke
	Scenario: Case Team user should be able to use search using Case Description - Open Cases
	...
	@CaseTeam @smoke
	Scenario: Case Team user should see an error when nothing is entered - Open Cases
