...
export class Page {
	...
	basePageElements = {
		...
		goToDashboardLink: () =>
			cy.contains(
				`${this.selectors.rightCol} ${this.selectors.link}`,
				"Go To Dashboard",
				{ matchCase: false }
			),
	...
	verifyCaseAdminIsSignedIn() {
		this.basePageElements
			.loggedInUser()
			.invoke("text")
			.should("equal", users.caseAdmin.typeName);
	}
	...
	verifyCaseTeamIsSignedIn() {
		this.basePageElements
			.loggedInUser()
			.invoke("text")
			.should("equal", users.caseTeam.typeName);
	}
	...
	verifyInspectorIsSignedIn() {
		this.basePageElements
			.loggedInUser()
			.invoke("text")
			.should("equal", users.inspector.typeName);
	}
}
