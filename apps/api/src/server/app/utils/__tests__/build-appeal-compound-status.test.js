...
test('when appeal status has single compound status throws error', (t) => {
	const error = t.throws(() => {
		buildAppealCompundStatus([{ status: 'first status', subStateMachineName: 'state_machine_1' }]);
	}, { instanceOf: Error });
...
test('when appeal status has single compound and single core status throws error', (t) => {
	const error = t.throws(() => {
		buildAppealCompundStatus([{ status: 'first status', subStateMachineName: 'state_machine_1' }, { status: 'second status' }]);
	}, { instanceOf: Error });
