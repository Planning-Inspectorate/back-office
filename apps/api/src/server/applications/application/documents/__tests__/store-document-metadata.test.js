...
describe('store Document metadata', () => {
	...
	test('The test checks if the endpoint returns a 400 bad request error when the request body has the wrong schema.', async () => {
		...
		expect(body).toEqual({
			errors:
				'"origin" must be one of [pins, citizen, lpa, ogd]. "redactedStatus" must be one of [not_redacted, redacted, ai_redaction_failed, awaiting_ai_suggestions, awaiting_ai_redaction, no_redaction_required, ai_suggestions_review_required, ai_suggestions_reviewed]. "published" is not allowed. "badFieldThatDoesNotExistOnSchema" is not allowed'
		});
