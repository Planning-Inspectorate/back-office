...
describe('Unpublish Representations', () => {
	...
	it('only processes representations that are currently PUBLISHED', async () => {
		const mixedStatusRepresentations = [
			{ ...representations[0], status: 'PUBLISHED' },
			{ ...representations[1], status: 'VALID' } // This one should be filtered out
		];

		databaseConnector.representation.findMany.mockResolvedValue(mixedStatusRepresentations);
		databaseConnector.case.findUnique.mockResolvedValue({ id: 1, reference: 'BC0110001' });
		...
		expect(response.status).toEqual(200);
		expect(response.body).toEqual({ unpublishedRepIds: [6409] }); // Only the PUBLISHED one

		// Should only process the PUBLISHED representation
		expect(databaseConnector.representation.update).toHaveBeenCalledTimes(1);
		expect(databaseConnector.representation.update).toHaveBeenCalledWith({
			where: {
				id: 6409
			},
