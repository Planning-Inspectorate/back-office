 ...
 * @returns {string}
 */
export const extractPublishedBlobName = (
	publishedDocumentURI,
	publishContainer = config.BLOB_PUBLISH_CONTAINER
) => {
	if (!publishedDocumentURI.includes(publishContainer)) {
		throw new Error(
			`invalid published URI, container not found: ${publishContainer} in ${publishedDocumentURI}`
		);
	}
