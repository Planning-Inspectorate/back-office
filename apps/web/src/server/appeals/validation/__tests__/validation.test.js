...
/** @typedef {import('../validation.validators').UnparsedInvalidOutcomeBody} InvalidOutcomeBody */
/** @typedef {import('../validation.validators').UnparsedIncompleteOutcomeBody} IncompleteOutcomeBody */
...
describe('validation', () => {
	...
	describe('GET /appeals/:appealId', () => {
		...
		it('should render an appeal in an incomplete state with the option to change the outcome', async () => {
			const { AppealId } = incompleteAppealDetails;
			const response = await request.get(`${baseUrl}/appeals/${AppealId}?edit=true`);
			const element = parseHtml(response.text);
	...
	describe('GET /appeals/:appealId/documents/:documentType', () => {
		...
		it('should render a page for uploading appeal letters', async () => {
			const response = await request.get(`${baseUrl}/appeals/${AppealId}/documents/appeal-letter`);
			const element = parseHtml(response.text);
		...
		it('should render a page for uploading a decision letter', async () => {
			const response = await request.get(
				`${baseUrl}/appeals/${AppealId}/documents/decision-letter`
			);
			const element = parseHtml(response.text);
		...
		it('should render a page for uploading the planning application form', async () => {
			const response = await request.get(
				`${baseUrl}/appeals/${AppealId}/documents/planning-application-form`
			);
			const element = parseHtml(response.text);
		...
		it('should render a page for uploading supporting documents', async () => {
			const response = await request.get(
				`${baseUrl}/appeals/${AppealId}/documents/supporting-document`
			);
			const element = parseHtml(response.text);
		...
		it('should redirect to the appeal when trying to add documents to a new appeal', async () => {
			const response = await request
				.get(
					`${baseUrl}/appeals/${receivedAppealDetails.AppealId}/documents/planning-application-form`
				)
				.redirects(1);
			const element = parseHtml(response.text);
	...
	describe('POST /appeals/:appealId/documents/:documentType', () => {
		...
		it('should validate that a file is not in excess of 15mb', async () => {
			const response = await request
				.post(`${baseUrl}/appeals/${AppealId}/documents/supporting-document`)
	...
	describe('GET /appeals/:appealId/appeal-site', () => {
		it('should render a page for editing the appeal site', async () => {
			const response = await request.get(
				`${baseUrl}/appeals/${incompleteAppealDetails.AppealId}/appeal-site`
			);
			const element = parseHtml(response.text);
	...
	describe('POST /appeals/:appealId/appeal-site', () => {
		const { AppealId } = incompleteAppealDetails;

		it('should validate that the required fields are present', async () => {
			const response = await request.post(`${baseUrl}/appeals/${AppealId}/appeal-site`).send(
				/** @type {Address} */ ({
					AddressLine1: ' ',
					Town: ' ',
					PostCode: ' '
		...
		it('should update the address site and return to the appeal page', async () => {
			...
			const response = await request
				.post(`${baseUrl}/appeals/${AppealId}/appeal-site`)
	...
	describe('GET /appeals/:appealId/appellant-name', () => {
		it('should render a page for editing the appellant name', async () => {
			const response = await request.get(
				`${baseUrl}/appeals/${incompleteAppealDetails.AppealId}/appellant-name`
			);
			const element = parseHtml(response.text);
	...
	describe('POST /appeals/:appealId/appellant-name', () => {
		const { AppealId } = incompleteAppealDetails;

		it('should validate that an appellant name is provided', async () => {
			const response = await request
				.post(`${baseUrl}/appeals/${AppealId}/appellant-name`)
	...
	describe('GET /appeals/:appealId/planning-application-reference', () => {
		it('should render a page for editing the planning application reference', async () => {
			const response = await request.get(
				`${baseUrl}/appeals/${incompleteAppealDetails.AppealId}/planning-application-reference`
			);
			const element = parseHtml(response.text);
	...
	describe('POST /appeals/:appealId/planning-application-reference', () => {
		const { AppealId } = incompleteAppealDetails;

		it('should validate that a planning application reference is provided', async () => {
			const response = await request
				.post(`${baseUrl}/appeals/${AppealId}/planning-application-reference`)
				.send(
	...
	describe('GET /appeals/:appealId/local-planning-department', () => {
		it('should render a page for editing the local planning department', async () => {
			const response = await request.get(
				`${baseUrl}/appeals/${incompleteAppealDetails.AppealId}/local-planning-department`
			);
			const element = parseHtml(response.text);
	...
	describe('POST /appeals/:appealId/local-planning-department', () => {
		const { AppealId } = incompleteAppealDetails;

		it('should validate that a local planning department is chosen', async () => {
			const response = await request
				.post(`${baseUrl}/appeals/${AppealId}/local-planning-department`)
	...
	describe('POST /appeals/:appealId', () => {
		describe('received appeal', () => {
			const { AppealId } = receivedAppealDetails;

			it('should validate the review outcome', async () => {
				const response = await request.post(`${baseUrl}/appeals/${AppealId}`);
				const element = parseHtml(response.text);
	...
	describe('GET /appeals/:appealId/review-outcome', () => {
		describe('received appeal', () => {
			const { AppealId } = receivedAppealDetails;

			it('should redirect when there is no review outcome in the session', async () => {
				const response = await request
					.get(`${baseUrl}/appeals/${AppealId}/review-outcome`)
					.redirects(1);
				const element = parseHtml(response.text);
			...
			it('should render a valid review outcome page', async () => {
				...
				const response = await request.get(`${baseUrl}/appeals/${AppealId}/review-outcome`);
				const element = parseHtml(response.text);
	...
	describe('POST /appeals/:appealId/review-outcome', () => {
		const { AppealId } = receivedAppealDetails;
		...
		describe('valid outcome', () => {
			...
			it('should validate that a description of development is provided', async () => {
				const response = await request.post(`${baseUrl}/appeals/${AppealId}/review-outcome`).send(
					/** @type {ValidAppealData} */ ({
						status: 'valid',
						descriptionOfDevelopment: ' '
					})
	...
	describe('POST /appeals/:appealId/review-outcome/confirm', () => {
		const { AppealId } = receivedAppealDetails;
		...
		describe('valid outcome', () => {
			...
			it('should confirm the outcome with a success panel', async () => {
				...
				const response = await request.post(
					`${baseUrl}/appeals/${AppealId}/review-outcome/confirm`
				);
				const element = parseHtml(response.text);
