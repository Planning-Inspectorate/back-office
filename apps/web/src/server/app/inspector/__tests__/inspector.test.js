...
describe('inspector', () => {
	...
	describe('POST /inspector/available-appeals', () => {
	...
	describe('GET /inspector/:appealId/book-site-visit', () => {
		it('should render a page for booking a site visit', async () => {
			const { appealId } = appealDetailsForUnbookedSiteVisit;
			const response = await request.get(`/inspector/appeals/${appealId}/book-site-visit`);
			const element = parseHtml(response.text);
	...
	describe('POST /inspector/:appealId/book-site-visit', () => {
		...
		it('should validate that the site visit details are provided', async () => {
			const response = await request.post(`/inspector/appeals/${appealId}/book-site-visit`);
			const element = parseHtml(response.text);
	...
	describe('GET /inspector/:appealId/confirm-site-visit', () => {
		...
		it('should render a page for confirming a site visit', async () => {
			const response = await request.get(`/inspector/appeals/${appealId}/confirm-site-visit`);
			const element = parseHtml(response.text);
	...
	describe('POST /inspector/:appealId/confirm-site-visit', () => {
		const { appealId } = appealDetailsForUnbookedSiteVisit;
	...
	describe('GET /inspector/:appealId/issue-decision', () => {
		...
		it('should render a page for issuing a decision', async () => {
			const { appealId } = appealDetailsForDecisionDue;
			const response = await request.get(`/inspector/appeals/${appealId}/issue-decision`);
			const element = parseHtml(response.text);
		...
		it('should redirect to the appeal page when issuing a decision is not eligible', async () => {
			const { appealId } = appealDetailsForUnbookedSiteVisit;
			const response = await request
				.get(`/inspector/appeals/${appealId}/issue-decision`)
				.redirects(1);
			const element = parseHtml(response.text);
	...
	describe('POST /inspector/:appealId/issue-decision', () => {
		beforeEach(() => {
			// Used to handle today's date being printed in the page
			installFixedDate(new Date(2023, 0, 1));
		});
		...
		it('should validate that the decision details are provided', async () => {
			const response = await request.post(`/inspector/appeals/${appealId}/issue-decision`);
			const element = parseHtml(response.text);
		...
		it('should validate that a file is not in excess of 15mb', async () => {
			const response = await request
				.post(`/inspector/appeals/${appealId}/issue-decision`)
	...
	describe('POST /inspector/:appealId/confirm-decision', () => {
		const { appealId } = appealDetailsForDecisionDue;
		...
		it('should confirm a decision and render a success page', async () => {
			...
			const response = await request.post(`/inspector/appeals/${appealId}/confirm-decision`);
			const element = parseHtml(response.text);
...
async function installAssignedAppealIds(appealIds) {
	...
	await request.post('/inspector/available-appeals').send({ appealIds });
...
async function installSiteVisit(appealId) {
	await request.post(`/inspector/appeals/${appealId}/book-site-visit`).send({
		siteVisitTimeSlot: '1pm to 3pm',
		siteVisitType: 'unaccompanied',
		'siteVisitDate-day': '1',
