...
import {
	...
	patchRepresentationRedact, REDACT_CHARACTER
...
export const getRedactRepresentationController = async (req, res) => {
	...
	let view = 'applications/representations/representation-details/redact-representation/redact-representation-legacy.njk';

	if (featureFlagClient.isFeatureActive('azure-ai-language-redaction')) {
		...
		if (redactionResult) {
			view = 'applications/representations/representation-details/redact-representation/redact-representation.njk';
			...
			if (viewModel.originalRepresentation === viewModel.redactedRepresentation) {
				...
				for (const redactionSuggestion of viewModel.redactionSuggestions) {
					...
					redactionSuggestion.accepted = viewModel.redactedRepresentation.charAt(redactionSuggestion.offset) === REDACT_CHARACTER;
				}
			}
			viewModel.originalRepresentation = highlightRedactionSuggestions(viewModel.originalRepresentation, viewModel.redactionSuggestions);
