...
export async function viewApplicationsCaseTimetableList({ session }, response) {
	...
	console.log('eyoo: ', examinationTimetable);
...
const getTimetableRows = (timetableItem) => {
	...
	const [localDate, localStartDate] = convertDatetimeToLocalTime(date, startDate);
	if (!localDate) {
		throw Error(
			`date field is missing from timetable item of type ${ExaminationTimetableType?.name}`
		);
	}
	...
	const {
		...
		formattedStartTime,
		formattedEndTime
	} = mapLocalTimeToDisplayFields(localDate, localStartDate);
	...
	return {
		...
		nameWelsh,
		submissions,
		date: requiredFields.includes('date') ? displayDate(formattedDate, { condensed: true }) : null,
		startDate: requiredFields.includes('startDate')
			? displayDate(formattedStartDate, { condensed: true })
			: null,
		endDate: requiredFields.includes('endDate')
			? displayDate(formattedEndDate, { condensed: true })
			: null,
