...
export const getRows = (tableData, helpers) => {
	...
	if (tableData.invoiceNumber) {
		rows = [
			[
				{
					text:
						helpers.getDisplayValue(helpers.invoiceStageDisplayValues, tableData.invoiceStage) || ''
				},
	...
	if (tableData.agenda) {
		rows = [
			[
				{ text: tableData.agenda || '' },
				{ text: helpers.formatDateForDisplay(tableData.meetingDate) || '' }
			]
		];
	}
