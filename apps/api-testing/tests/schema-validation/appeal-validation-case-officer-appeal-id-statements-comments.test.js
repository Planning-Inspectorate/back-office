...
describe('Appeals - Case Officer By Appeal ID', () => {
	it('Schema Validation', async () => {
		const endpoint = '/appeals/case-officer/{appealId}/statements-comments';
		const { body: appeals } = await new TestService('/appeals/validation').get();
		const promises = appeals.map(async (appeal) => {
			const request = new TestService(
				`/appeals/case-officer/${appeal.AppealId}/statements-comments`
			);
