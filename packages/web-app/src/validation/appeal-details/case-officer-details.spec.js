...
const { rules } = require('./case-officer-details');

describe('validators/case-officer-details', () => {
  describe('rules', () => {
    it(`has a rule for the appellant's name`, () => {
      const rule = rules()[0].builder.build();

      expect(rule.fields).toEqual(['case-officer-name']);
      expect(rule.locations).toEqual(['body']);
      // expect(rule.optional).toBeFalsy();
      // expect(rule.optional).toBeNull();
      ...
      expect(rule.stack[0].validator.name).toEqual('matches');
      expect(rule.stack[0].options[0]).toEqual(/^[a-z\-' ]+$/i);
    ...
    //
    //   expect(rule.fields).toEqual(['case-officer-email']);
    //   expect(rule.locations).toEqual(['body']);
    //   expect(rule.optional).toBeFalsy();
    ...
    it('should have an array of rules', () => {
      ...
      expect(rules()[0].builder.fields[0]).toEqual('case-officer-name');
      expect(rules()[1].builder.fields[0]).toEqual('case-officer-email');
    });
  });
  ...
  describe('validator', () => {
    [
      ...
      // },
      {
        ...
        given: () => ({
          body: {
            'case-officer-name': '12 abc',
            'case-officer-email': '@.com',
          },
        }),
        expected: (result) => {
      ...
      },
      {
        ...
        given: () => ({
          body: {
            'case-officer-name': "timmy o'tester-jones",
            'case-officer-email': 'thomas-@example.com',
          },
        }),
        expected: (result) => {
          expect(result.errors).toHaveLength(1);
      ...
      },
      {
        ...
        given: () => ({
          body: {
            'case-officer-name': "timmy o'tester-jones",
            'case-officer-email': 'thomas@example.c',
          },
        }),
        expected: (result) => {
          expect(result.errors).toHaveLength(1);
      ...
      },
      {
        ...
        given: () => ({
          body: {
            'case-officer-name': "timmy o'tester-jones",
            'case-officer-email': 'timmy@example.com',
          },
        }),
        expected: (result) => {
