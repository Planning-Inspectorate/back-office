...
const getFailedSubCheckboxBoxAreaValidationTest = (data) => ({
  title: `lpaqreview-${data.title}-notification-checkbox - fail`,
  given: () => ({
    body: {
      [`lpaqreview-${data.title}-notification-checkbox`]: 'on',
      [`lpaqreview-${data.title}-notification-subcheckbox1`]: '',
      [`lpaqreview-${data.title}-notification-subcheckbox2`]: '',
    },
  }),
  expected: (result) => {
    expect(result.errors).toHaveLength(1);
    ...
    expect(result.errors[0].param).toEqual(`lpaqreview-${data.title}-notification-checkbox`);
    expect(result.errors[0].value).toEqual('on');
  },
});
