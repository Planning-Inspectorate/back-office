...
const { getDocumentsMetadata, uploadDocument } = require('./documents-api-wrapper');
...
describe('lib/documents-api-wrapper', () => {
  ...
  describe('uploadDocument', () => {
    const file = { name: 'PDF Test.pdf' };

    it('should return the correct data when a successful response is received', async () => {
      const response = await uploadDocument(appealId, file);
    ...
    it('should throw an error when an error occurs', () => {
      expect(() => uploadDocument(appealId, file)).rejects.toThrow(
        'Failed to upload document - Error: Internal Server Error'
      );
    });
  });
});
