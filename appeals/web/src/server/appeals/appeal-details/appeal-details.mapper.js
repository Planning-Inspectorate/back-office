...
export async function appealDetailsPage(appealDetails, currentRoute, session) {
	...
	if (appealDetails.appealStatus === 'issue_determination') {
		...
		session.notificationBanners.readyForDecision = {
			...
			titleText: 'Important',
			html: `<p class="govuk-notification-banner__heading">The appeal is ready for a decision.</p> <p class="govuk-notification-"><a href="/appeals-service/appeal-details/${appealDetails.appealId}/issue-decision/decision" class="govuk-link">Make a decision</a></p>`
		};
	}
