...
export function addDocumentDetailsPage (backLinkUrl, folder, bodyItems) {
	const unpublishedDocuments = folder.documents.filter((document) => document.latestDocumentVersion?.published === false);
	...
	const pageContent = {
		...
		pageComponentGroups: unpublishedDocuments
			.map((document, index) => ({
				wrapperHtml: {
					opening: `<div class="govuk-form-group"><h2 class="govuk-heading-m">${document.name}</h2>`,
					closing: index < unpublishedDocuments.length - 1 ? '<hr class="govuk-!-margin-top-7"></div>' : '</div>'
				},
				pageComponents: [
					{
						type: 'input',
 ...
 */
export async function manageDocumentPage (backLinkUrl, redactionStatuses, document, folder, session) {
	/** @type {PageContent} */
	const pageContent = {
		title: 'Manage documents',
		backLinkText: 'Back',
		...
		pageComponentGroups: [
			...
			{
				wrapperHtml: {
					opening: '<div class="govuk-grid-row"><div class="govuk-grid-column-full"><h2>Document history</h2><p class="govuk-body">See version history, a change log or remove old documents</p>',
					closing: '</div></div>'
				},
				pageComponents: [
					{
	...
	pageContent.pageComponentGroups?.forEach(group => preRenderPageComponents(group.pageComponents));
