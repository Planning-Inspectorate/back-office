	...
	for (const pageComponent of pageComponents) {
		...
		for (const parametersKey of parametersKeys) {
			...
			if (Array.isArray(parametersProperty)) {
				for (const item of parametersProperty) {
					...
					for (const itemKey of itemKeys) {
						...
						if (itemKey === 'html' && 'pageComponents' in item) {
							...
							preRenderPageComponents(itemProperty.pageComponents, recursions + 1, maximumRecursions);
				...
				preRenderPageComponents(pageComponent.parameters.pageComponents, recursions + 1, maximumRecursions);
				pageComponent.parameters.html = renderPageComponentsToHtml(pageComponent.parameters.pageComponents);
			}
		}
	}
};
...
export function renderPageComponentsToHtml (pageComponents) {
	return pageComponents
		.map(pageComponent => nunjucks.render('appeals/components/page-component.njk', { component: pageComponent }).trim())
		.join('');
}
